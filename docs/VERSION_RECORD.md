...
- 日期：2026-08-10
- 分支：`main`
- 自动防御：新增抓包入口高置信静默快速封禁层，典型攻击请求无需等待模型、批次或态势形成即可进入异步 Windows 双向防火墙处置。
- 链路兼容：快速规则命中不创建攻击事件或态势告警，同一请求仍保存到 `input` 并继续运行 Payload、POC、行为窗口、态势和 LLM/RAG，未改变原有告警链路。
- 安全边界：仅自动处理公网来源，拒绝回环、私网、链路本地和保留地址；普通登录、正常目录访问、普通搜索和文档代码样本均纳入负样本回归。
- 内部审计：新增 `demo_fast_defense_audit`，不提供前端接口；封禁状态复用现有统一封禁表和解封能力。
- 验证：85 项项目级自动化测试全部通过；27 条规则成功加载，10 类典型攻击正样本命中，7 类正常业务负样本不命中；本机 1 万次混合判定平均约 `0.145 ms/次`。
